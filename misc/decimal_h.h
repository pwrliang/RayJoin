...
inline ARIES_HOST_DEVICE_NO_INLINE int GetDecimalRealBytes(uint16_t precision,
                                                           uint16_t scale) {
...
inline ARIES_HOST_DEVICE_NO_INLINE int aries_is_space(int ch) {
  return (unsigned long) (ch - 9) < 5u || ' ' == ch;
}

inline ARIES_HOST_DEVICE_NO_INLINE int aries_atoi(const char* str,
                                                  const char* end) {
  int sign;
  int n = 0;
  const char* p = str;
...
inline ARIES_HOST_DEVICE_NO_INLINE int64_t aries_atol(const char* str,
                                                      const char* end) {
  int sign;
  int64_t n = 0;
  const char* p = str;
...
inline ARIES_HOST_DEVICE_NO_INLINE int aries_strlen(const char* str) {
  const char* p = str;
  while (*p++)
    ;
...
inline ARIES_HOST_DEVICE_NO_INLINE char* aries_strcpy(char* strDest,
                                                      const char* strSrc) {
  if (strDest == strSrc) {
    return strDest;
  }
...
inline ARIES_HOST_DEVICE_NO_INLINE char* aries_strncpy(char* strDest,
                                                       const char* strSrc,
                                                       unsigned int count) {
  if (strDest == strSrc) {
    return strDest;
...
inline ARIES_HOST_DEVICE_NO_INLINE char* aries_strcat(char* strDes,
                                                      const char* strSrc) {
...
inline ARIES_HOST_DEVICE_NO_INLINE char* aries_strncat(char* strDes,
                                                       const char* strSrc,
                                                       unsigned int count) {
  assert((strDes != NULL) && (strSrc != NULL));
  char* address = strDes;
...
inline ARIES_HOST_DEVICE_NO_INLINE char* aries_strchr(const char* str, int ch) {
...
inline ARIES_HOST_DEVICE_NO_INLINE char* aries_sprintf(char* dst,
...
inline ARIES_HOST_DEVICE_NO_INLINE void* aries_memset(void* dst, int val,
...
inline ARIES_HOST_DEVICE_NO_INLINE void* aries_memcpy(void* dst,
                                                      const void* src,
                                                      unsigned long ulcount) {
  if (!(dst && src))
    return 0;
...
inline ARIES_HOST_DEVICE_NO_INLINE int aries_strcmp(const char* source,
                                                    const char* dest) {
  int ret = 0;
  if (!source || !dest)
    return -2;
...
inline ARIES_HOST_DEVICE_NO_INLINE char* aries_strstr(const char* strSrc,
                                                      const char* str) {
...
inline ARIES_HOST_DEVICE_NO_INLINE char* aries_strerase(char* strSrc, int n) {
...
inline ARIES_HOST_DEVICE_NO_INLINE int32_t abs_cmp(int32_t* a,
                                                   const int32_t* b) {
  int32_t res = 0;
#pragma unroll
...
inline ARIES_DEVICE void abs_lshift(uint32_t* a, int len, int n,
                                    uint32_t* res) {
...
struct ARIES_PACKED Decimal {
 ...
 public:
  ...
  ARIES_HOST_DEVICE_NO_INLINE Decimal(uint32_t precision, uint32_t scale,
    ...
    // 对 this 初始化精度信息
    initialize(precision, scale, 0);
    // 根据字符串 s 所提供的信息，构造 Decimal d，此时 d
    // 的精度信息为初始精度信息
    Decimal d(s);
  ...
  ARIES_HOST_DEVICE_NO_INLINE Decimal(const CompactDecimal* compact,
                                      ...
                                      uint32_t m) {
    initialize(precision, scale, 0);
    int len = impl::GetDecimalRealBytes(precision, scale);
    impl::aries_memcpy((char*) (v), compact->data, len);
    char* temp = ((char*) (v));
    temp += len - 1;
  ...
  ARIES_HOST_DEVICE_NO_INLINE bool ToCompactDecimal(char* buf, int len) {
    impl::aries_memcpy(buf, (char*) (v), len);
  ...
  ARIES_HOST_DEVICE_NO_INLINE char* GetPrecisionScale(char result[]) {
    ...
    int prec0 = prec;
    int frac0 = frac;
    impl::aries_sprintf(temp, "%d", prec0);
    impl::aries_strcpy(result, temp);
    impl::aries_strcat(result, ",");
    impl::aries_sprintf((char*) temp, "%d", frac0);
    impl::aries_strcat(result, temp);
    return result;
  }

  // 返回错误代码
  ARIES_HOST_DEVICE_NO_INLINE uint16_t GetError() { return error; }
...
#pragma unroll
  ...
  ARIES_HOST_DEVICE_NO_INLINE Decimal& cast(const Decimal& t) {
    ...
    if (frac >= t.frac) {
      ...
      // 需要扩充
      if (shift != 0) {
        impl::aries_memcpy(v, t.v, sizeof(t.v));
        *this << shift;
      }
      // 不需要扩充
      else {
    ...
    //
      ...
      *this >> shift;
      ...
      //         asm volatile ("addc.cc.u32 %0, %1, %2;" : "=r"(v[i]) :
      //         "r"(v[i]), "r"(0));
      // }
      CheckOverFlow();
    }
    ...
    if (p > 0) {
      ...
      if (intg > p) {
        ...
        *this << p;
        ...
        impl::aries_memset(v, 0x00, sizeof(v));
        prec = 1;
      }
      frac = 0;
    }
  ...
  ARIES_HOST_DEVICE_NO_INLINE operator bool() const { return !isZero(); }

  ARIES_HOST_DEVICE_NO_INLINE operator double() const { return GetDouble(); }
...
#pragma unroll
  ...
  // double / float
  ARIES_DEVICE double operator+=(const float& f) { return *this += (double) f; }

  ARIES_DEVICE double operator+=(const double& l) { return GetDouble() + l; }
...
#pragma unroll
  ...
  // double / float
  ARIES_DEVICE double operator-=(const float& f) { return GetDouble() - f; }
  ...
  friend ARIES_DEVICE double operator-(const Decimal& left,
                                       const double right) {
    return left.GetDouble() - right;
  }
  ...
  friend ARIES_DEVICE double operator-(const double left,
                                       const Decimal& right) {
    return left - right.GetDouble();
  }
  // for multiple
...
#pragma unroll
  ...
  ARIES_DEVICE double operator*=(const double& d) { return GetDouble() * d; }
  // two operators
  friend ARIES_DEVICE Decimal operator*(const Decimal& left,
                                        const Decimal& right);
  // signed
  ...
  ARIES_DEVICE Decimal& DivOrMod(const Decimal& d, bool isMod = false) {
    ...
    // 对于unsign int类型数据的处理,这里的除数都应该是unsign int类型数据
      ...
      for (; tmpEx > 0;) {
        ...
        // 那么最大可 左移（十进制）的数量是 没用到的uint32 * 9 + 9
        // -用到的第一个 uint32 已用的十进制空间
        while (tmp > 0) {
          tmp /= 10;
          nDigits--;
        }
...
#pragma unroll
  ...
  ARIES_DEVICE Decimal& operator/=(const Decimal& d) { return DivOrMod(d); }
  ...
  // 取余操作
  ARIES_DEVICE Decimal& operator%=(const Decimal& d) { return *this; }
  // signed
  ARIES_DEVICE Decimal& operator%=(int8_t i) { return *this; }

  ARIES_DEVICE Decimal& operator%=(int16_t i) { return *this; }

  ARIES_DEVICE Decimal& operator%=(int32_t i) { return *this; }

  ARIES_DEVICE Decimal& operator%=(int64_t i) { return *this; }

  // unsigned
  ARIES_DEVICE Decimal& operator%=(uint8_t i) { return *this; }

  ARIES_DEVICE Decimal& operator%=(uint16_t i) { return *this; }

  ARIES_DEVICE Decimal& operator%=(uint32_t i) { return *this; }

  ARIES_DEVICE Decimal& operator%=(uint64_t i) { return *this; }

  // double % float
  ARIES_DEVICE double operator%=(const float& f) { return 0.0; }

  ARIES_DEVICE double operator%=(const double& d) { return 0.0; }
...
#pragma unroll
      for (int j = 0; j < __ARRAY_SCALE[n - 1]; j++) {
        ...
        temp = (uint64_t) v[i] * (uint64_t) __POW10_ARRAY[n - 1][j] +
               res[i + j] + carry;
        carry = temp >> 32;
        res[i + j] = temp & 0x00000000ffffffff;
      }
...
#pragma unroll
  ...
  // 无用函数
  ARIES_HOST_DEVICE_NO_INLINE Decimal& GenIntDecimal(int shift) {
    return *this;
  }
  // 将两个整数decimal折半取值
...
#pragma unroll
  ...
  ARIES_DEVICE Decimal DivInt(const Decimal ds, const Decimal dt,
    ...
    // 被除数为0
    if (ds.isZero()) {
      impl::aries_memset(residuel.v, 0x00, sizeof(residuel.v));
      return ds;
    }
    // 通过二进制的方法得出数据上下限
    uint32_t dsz = INDEX_LAST_DIG;
    ...
    // 2^(dtHitBit-1) -> 2^(dsHitBit-dtHitBit+1) 下限 2^(dsHitBit-1) /
    // 2^dtHitBit
  ...
  // 根据精度获取其最大值 TODO 只在检测整数部分也溢出后采用的方法 考虑只设置
  // error 位 不求该范围下的最大值 外面没有调用此函数
  ...
  ARIES_DEVICE Decimal& DivByInt(const Decimal& d, int shift,
    ...
    *this << shift;
    for (int i = NUM_TOTAL_DIG - 1; i >= 0; i--) {
      if (v[i] || remainder) {
        uint64_t tmp =
            (uint64_t) v[i] + (uint64_t) remainder * PER_DEC_MAX_SCALE;
        v[i] = tmp / dvs;
        remainder = tmp % dvs;
      }
    }
...
#pragma unroll
  ...
  // int64相除
  ARIES_DEVICE Decimal& DivByInt64(const Decimal& divisor, int shift,
                                   bool isMod = false) {
    // 被除数 在 int64 范围内
...
// global method
ARIES_HOST_DEVICE_NO_INLINE aries_acc::Decimal abs(aries_acc::Decimal decimal) {
  decimal.sign = 0;
  return decimal;
}
