...
__device__ inline uint2 determine_range(UInt const* node_code,
...
template <typename UInt>
__device__ inline unsigned int find_split(UInt const* node_code,
                                          const unsigned int num_leaves,
...
struct default_morton_code_calculator {
  ...
  default_morton_code_calculator& operator=(
      default_morton_code_calculator const&) = default;
  default_morton_code_calculator& operator=(default_morton_code_calculator&&) =
      default;

  __device__ __host__ inline unsigned int operator()(
      const Object&, const aabb<Real>& box) noexcept {
    auto p = centroid(box);
    p.x -= whole.lower.x;
    p.y -= whole.lower.y;
    p.z -= whole.lower.z;
...
template <typename Real, typename Object, typename AABBGetter,
          typename MortonCodeCalculator =
              default_morton_code_calculator<Real, Object>>
class bvh {
 public:
  using real_type = Real;
  using index_type = std::uint32_t;
 ...
 public:
  bvh(const thrust::device_vector<object_type>& objects,
      bool query_host_enabled = false)
      : objects_d_(objects), query_host_enabled_(false) {
    this->construct();
  }

  bvh(const pinned_vector& objects, bool query_host_enabled = false)
      : objects_d_(objects), query_host_enabled_(false) {
    this->construct();
  }
  ...
  void assign(const pinned_vector& objects) { this->objects_d_ = objects; }
  ...
  bvh_device<real_type, object_type> get_device_repr() noexcept {
    return bvh_device<real_type, object_type>{
        ...
        static_cast<unsigned int>(objects_d_.size()), nodes_.data().get(),
  ...
  thrust::host_vector<aabb_type> const& aabbs_host() const noexcept {
    return aabbs_h_;
  }
